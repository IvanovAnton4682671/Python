...
def small_error(x, matrix, answ_list, x_list):
    ...
    res = 0
    for j in range(len(matrix[x - 1])):
        list_res.append(matrix[x - 1][j] * x_list[j])
    for elem in list_res:
        res += elem
    res -= answ_list[x - 1]
    return abs(res)

def true_error(matrix, answ_list, x_list):
    res = numpy.dot(matrix, x_list) - answ_list
    return res

def task_a():
    #matrix = [[0.0001, 1], [1, 2]]
    #answ_list = [1, 4]
    matrix = [[3, 2, -5], [2, -1, 3], [1, 2, -1]]
    matrix2 = matrix
    answ_list = [-1, 13, 9]
    x1, x2, x3 = gauss(matrix, answ_list)
    x_list = [x1, x2, x3]
    print("Задача а): Х1 = {}, Х2 = {}, X3 = {}".format(round(x1, 5), round(x2, 5), round(x3, 5)))
    ...
    print("Невязки (по строкам): 1 = {}, 2 = {}, 3 = {}".format(small_error(1, matrix2, answ_list, x_list),
                                                                small_error(2, matrix2, answ_list, x_list),
                                                                small_error(3, matrix2, answ_list, x_list)))
    print("Правильные невязки: {}".format(true_error(matrix, answ_list, x_list)))
